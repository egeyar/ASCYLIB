...
bool_t bst_insert(skey_t key, sval_t val, node_t* node_r) {
    ...
    MEM_BARRIER;
#endif
        node_t* result = ATOMIC_CAS_TSX(child_addr, ADDRESS(leaf), ADDRESS(new_internal));
    ...
    while (1) {
      UPDATE_TRY();
        ...
        if (injecting == TRUE) {
            ...
            node_t* result = ATMOIC_CAS_TSX(child_addr, lf, FLAG(lf));
            if (result == ADDRESS(leaf)) {
                injecting = FALSE;
...
//#if defined(__tile__) || defined(__sparc__)
    while (1) {
        ...
        node_t* res = ATOMIC_CAS_TSX(sibling_addr,untagged, tagged);
        if (res == untagged) {
            break;
         }
    }
...
//#endif
    ...
    if ( ATOMIC_CAS_TSX(succ_addr, ADDRESS(successor), UNTAG(sibl)) == ADDRESS(successor)) {
#if GC == 1
