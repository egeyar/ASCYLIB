...
physical_delete_right(node_t* left_node, node_t* right_node) 
{
  ...
  node_t* res = (node_t*) ATOMIC_CAS_TSX64(&left_node->next, right_node, new_next);
...
harris_insert(intset_t *the_list, skey_t key, sval_t val)
{
  ...
  do
      ...
      if (right_node->key == key) 
	{
      ...
      cas_result = (node_t*) ATOMIC_CAS_TSX64(&left_node->next, right_node, node_to_add);
...
harris_delete(intset_t *the_list, skey_t key)
{
  ...
  do
      ...
      if (right_node->key != key)
	{
      ...
      cas_result = (node_t*) ATOMIC_CAS_TSX64(&right_node->next, unmarked_ref, marked_ref);
    } 
