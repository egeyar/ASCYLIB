...
bool_t bst_insert(skey_t key, sval_t val, node_t* node_r) {
    ...
    MEM_BARRIER;
#endif
        node_t* result = (node_t*) ATOMIC_CAS_TSX64(child_addr, ADDRESS(leaf), ADDRESS(new_internal));
    ...
    while (1) {
      UPDATE_TRY();
        ...
        if (injecting == TRUE) {
            ...
            node_t* result = (node_t*) ATOMIC_CAS_TSX64(child_addr, lf, FLAG(lf));
            if (result == ADDRESS(leaf)) {
                injecting = FALSE;
...
//#if defined(__tile__) || defined(__sparc__)
    ...
    TSX_CRITICAL_SECTION {
        ...
        *sibling_addr = tagged;
        *succ_addr = (node_t*) UNTAG(*sibling_addr);
        TSX_COMMIT;
#if GC == 1
    ...
    while (1) {
        ...
        tagged = (node_t*)TAG(untagged);
        node_t* res = (node_t*) ATOMIC_CAS_TSX64(sibling_addr,untagged, tagged);
        if (res == untagged) {
            break;
         }
    }
...
//#endif
    ...
    if ( (node_t*) ATOMIC_CAS_TSX64(succ_addr, ADDRESS(successor), UNTAG(sibl)) == ADDRESS(successor)) {
#if GC == 1
    ssmem_free(alloc, ADDRESS(chld));
    ssmem_free(alloc, ADDRESS(successor));
#endif
