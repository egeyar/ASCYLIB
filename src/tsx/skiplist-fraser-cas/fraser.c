 ...
 retry:
  ...
  for (i = levelmax - 1; i >= 0; i--)
      ...
      for (right = left_next; ; right = right_next)
	{
      ...
      if ((left_next != right))
	{
	  if ((!ATOMIC_CAS_TSX(&left->next[i], left_next, right)))
	    {
	      CLEANUP_TRY();
	      goto retry;
	    }
  ...
  for (i = n->toplevel - 1; i >= 0; i--)
    {
      ...
      while (!ATOMIC_CAS_TSX(&n->next[i], n_next, set_mark((uintptr_t)n_next)));
    }
}
...
fraser_insert(sl_intset_t *set, skey_t key, sval_t val) 
{
 ...
 retry: 	
  ...
  if (!ATOMIC_CAS_TSX(&preds[0]->next[0], succs[0], new))
    {
      goto retry;
    }
  ...
  for (i = 1; i < new->toplevel; i++) 
      ...
      while (1) 
	{
	  ...
	  if ((new_next != succ) && 
	      (!ATOMIC_CAS_TSX(&new->next[i], unset_mark((uintptr_t)new_next), succ)))
	  ...
	  if (ATOMIC_CAS_TSX(&pred->next[i], succ, new))
	    break;
